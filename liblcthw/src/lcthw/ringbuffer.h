...
// #define RingBuffer_available_data(B) (((B)->end + 1) % (B)->length - (B)->start - 1) // unvalid when pointer wrap_around happened 

// #define RingBuffer_available_space(B) ((B)->length - (B)->end - 1)

#define RingBuffer_available_data(B) (((B)->end >= (B)->start) ? ((B)->end - (B)->start) : ((B)->length - (B)->start + (B)->end))

//#define RingBuffer_available_space(B) (RingBuffer_available_data(B) - 1)
...
#define RingBuffer_starts_at(B) ((B)->buffer + (B)->start)//the macro used to get the current position from which data can be read

#define RingBuffer_ends_at(B) ((B)->buffer + (B)->end)//get the position which data can be written 
