...
RingBuffer *RingBuffer_create(int length)
{
    ...
    buffer->length  = length + 1;// empty state is start = end; full state is end +1 = start
    buffer->start = 0;
    buffer->end = 0;
    buffer->buffer = calloc(buffer->length, 1); //void *calloc(size_t nmemb, size_t size);
...
int RingBuffer_write(RingBuffer *buffer, char *data, int length)
{
    ...
    RingBuffer_commit_write(buffer, length);// update the (end) pointer
